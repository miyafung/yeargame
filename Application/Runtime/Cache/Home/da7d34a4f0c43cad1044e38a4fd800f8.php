...
<title>飞宇抽奖</title>
<link href="/nianhui/Public/Common/css/sweet-alert.css" rel="stylesheet">
...
<script src="/nianhui/Public/Common/bootstrap/js/bootstrap.min.js"></script>
<script src='/nianhui/Public/Common/js/prefixfree.min.js'></script>
<script src='/nianhui/Public/Common/js/sweet-alert.min.js'></script>
<style class="cp-pen-styles">
	...
	.container { width: 1900px; position: absolute; top: 50%; left: 50%; margin-left: -950px; margin-top: -476px; }
	.prizeBox,.orderBox { position: relative; /*border: 2px solid #0b819b;*/ float: left; background-color: rgba(51,51,51,0.3); }
	.prizeTitle { width: 690px; position: absolute; top: -52px; left: 50%; margin-left: -345px; }
	.prizeBox { width: 690px; height: 1000px; margin-right: 40px; }
	.orderBox { width: 1140px; height: 1000px; }
    .prizeListOrder { margin-top: 140px; }
	.joinNumb { text-align: center; color: #eee; font-size: 12px; margin-top: 30px; }
	.prizeName { width: 330px; height: 330px; margin: 164px auto 80px; line-height: 330px; text-align: center; font-size: 50px; color: #eee; position: relative;}
    .prizeInfo { position: relative; }
    .prizeBgTurn { position: absolute; top: -88px; left: 90px; z-index: 0; width: 510px; }
    .removeC { animation: a 50s; }
    .nextPrize { margin: 0 auto; width: 164px; font-size: 36px;  }
	...
	.prizeThing { width: 520px; background-color: #bbb; border: 1px solid #eee; padding: 10px; margin: 24px auto 0; border-radius: 30px; }
	.prizeLeft,.prizeRight { display: inline-block; width: 50px; height: 50px; font-size: 50px; cursor: pointer; }
	.prizeThingName { width: 380px; display: inline-block; text-align: center; height: 50px; line-height: 50px; font-size: 50px; }
	.prizeJoinNumber { color: #eee; text-align: center; margin: 26px 0; font-size: 20px; }
	.prizeBtn { width: 430px; margin: 70px auto 0;  }
	.prizeBtn button { width: 370px; background-color: #ff9c00; color: #eee; text-align: center; height: 70px; line-height: 70px; border: 1px solid #ff9c00; margin: 30px auto 0; display: block; font-size: 40px; }
	.orderBox { color: #eee; }
    .prizeListOrder .current { display: block; }
	.prizeGetBox { width: 1100px; margin: 60px auto 0; /*padding: 20px;*/ display: none;  }
    #cusList0,#cusList1{ padding: 0 360px; }
	.prizeGetBox li { float: left; margin: 90px 0; width: 274px; text-align: center; }
    .prizeGetBox li img { width: 170px; margin-bottom: 30px; }
    .prizeGetBox li p { font-size: 36px; }

    #cusList0 li img,#cusList1 li img { width: 270px; }
    #cusList0 li,#cusList1 li { width: 370px; }
    #cusList0 li p,#cusList1 li p{ font-size: 36px; }

    #cusList2,#cusList3,#cusList4,#cusList5,#cusList6 { padding: 0 230px; }
    #cusList2 li img,#cusList3 li img,#cusList4 li img,#cusList5 li img,#cusList6 li img { width: 210px; }
    #cusList2 li,#cusList3 li,#cusList4 li,#cusList5 li,#cusList6 li { width: 300px;}
    #cusList2 li p,#cusList3 li p,#cusList4 li p,#cusList5 li p,#cusList6 li p { font-size: 50px}

    #cusList8 li,#cusList9 li { width: 346px; }
    #cusList8 li img,#cusList9 li img { width: 196px; }

    #cusList10 li,#cusList11 li,#cusList12 li,#cusList13 li{ width: 220px;}
    #cusList10 li img,#cusList11 li img,#cusList12 li img,#cusList13 li img,#cusList14 li img,#cusList15 li img { width: 150px; }

    #cusList14 li p,#cusList15 li p { font-size: 32px; }
    #cusList14 li,#cusList15 li { width: 200px; margin: 20px 10px 90px; }
	h4 { text-align: center; margin: 50px 0; font-size: 40px; }
...
<canvas id="canvas"></canvas>
	...
	<div class="container">
		<div class="prizeBox">
			<img class="prizeTitle" src="/nianhui/Public/Home/img/prizeTop.png">
			<!-- <p class="joinNumb">参加抽奖人数: 243 </p> -->
			<div class="prizeInfo">
                <img class="prizeBgTurn" src="/nianhui/Public/Home/img/bg.png">
				<div class="prizeName">
                    <img src="/nianhui/Public/Home/img/10.png" width="330">
                </div>
				<div class="joinPhone">
                    <ul class="nextPrize">
                        <li>现金大奖</li>
		...
		<div class="orderBox">
			<img class="prizeTitle" src="/nianhui/Public/Home/img/orderTop.png">
	...
	<script src='/nianhui/Public/Common/js/stopExecutionOnTimeout.js'></script>
...
<script>
    ...
    function prizeArrList(){
        for(var z=0; z<prizeArr.length; z++){
            if(prizeArr[z]['name'] == $('.nextPrizeCurrent').text()){
                ...
                $('.prizeName').text('');
                $('.prizeName').append('<img src='+prizeArr[z]['url']+' width="330">');
    ...
    function prizeThingList(){
        ...
        switch(awardNameB){
            ...
            case '三等奖':
                nextPrizeWidth(590);
                $('.nextPrize').append('<li>小米电视55寸</li><li>iPad</li><li>华为手机</li>');
                break;
            case '四等奖':
                nextPrizeWidth(636);
    ...
    //奖品图片路径
    function pathUrl(a){
        return '/nianhui/Public/Home/img/'+a+'.png';
    }
    ...
    function addHtml(a,b,c){
        ...
        if(c == 0){
            $('#cusList'+a).append('<li><img src="/nianhui/Public/Home/img/person1'+randNumber+'.png"><p>'+ b['name'] + '</p></li>');
        }else{
            $('#cusList'+a).append('<li><img src="/nianhui/Public/Home/img/person2'+randNumber+'.png"><p>'+ b['name'] + '</p></li>');
        }
    ...
    function postAjax(name,num,awardName){
        var url = '/nianhui/Home/Index/eidtThing';   //初始化链接
    ...
    var prizeArr = [
        ...
        {name:'iPad',num:2,url:pathUrl(32),state:32},
    ...
    console.log(mobile.length+'|'+white.length+'|'+white2.length);
    var count = target.length;
    //console.log(white.length+'|'+white2.length);
    if(count == 0){
        $('.prizeName').text('');
        $('.prizeName').append('<img src="/nianhui/Public/Home/img/10.png" width="330">');
    }else{
        for(var i=0; i < count; i++){
            for(var p =0; p<prizeArr.length; p++){
                if(target[i]['spoil'] == prizeArr[p]['name']){
                    ...
                    var randNumber = GetRnd(0,8);
                    if(target[i]['sex'] == 0){
                        cusListB = '<img src="/nianhui/Public/Home/img/person1'+randNumber+'.png">';
                    }else{
                        cusListB = '<img src="/nianhui/Public/Home/img/person2'+randNumber+'.png">';
                    }
    ...
    /*for(var g =0; g<prizeArr.length; g++){
        console.log($('#cusList'+g).length + '|');
        if($('#cusList'+g).length >0){
            for(var f=0; f<$('#cusList'+g).length; f++){
                cusListB = '<img src="/nianhui/Public/Home/img/person'+f+'.png">';
                $('#cusList'+g).children('li').eq(f).prepend(cusListB);
            }
        }
    ...
    $('#reset').click(function(){
        var url = '/nianhui/Home/Index/index'; 
    ...
    function stop() {
        ...
        for(var t = 0; t < prizeArr.length; t++){
            if(prizeArr[t]['name'] == $('.prizeX').text()){
                $('.prizeName').prepend('<img src="'+prizeArr[t]['url']+'" width="330">');
            }
        }
